...
namespace wowpp
{
	MailSender::MailSender(GameObject* sender, MailStationery stationery) : m_stationery(stationery)
	{
		switch (sender->getTypeId())
		{
		case object_type::Unit:
			m_messageType = mail_message_type::Creature;
			m_senderId = sender->getUInt32Value(object_fields::Entry);
			break;
		case object_type::GameObject:
			m_messageType = mail_message_type::GameObject;
			m_senderId = sender->getUInt32Value(object_fields::Entry);
			break;
		case object_type::Item:
		case object_type::Container:
			m_messageType = mail_message_type::Item;
			m_senderId = sender->getUInt32Value(object_fields::Entry);
			break;
		case object_type::Character:
			m_messageType = mail_message_type::Normal;
			m_senderId = guidLowerPart(sender->getGuid());
			break;
		default:
			m_messageType = mail_message_type::Normal;
			m_senderId = 0;                                 // will show mail from nonexistent player
			ELOG("MailSender::MailSender - Mail have unexpected sender typeid (%u)", sender->getTypeId());
			break;
		}
	}
	...
	*/
	MailReceiver::MailReceiver(Player* receiver) : m_receiver(receiver), m_receiver_guid(receiver->getCharacterGuid())
	{
	}
	/**
	...
	*/
	MailReceiver::MailReceiver(Player* receiver, UInt64 receiver_guid) : m_receiver(receiver), m_receiver_guid(receiver_guid)
	{
		assert(!receiver || receiver->getCharacterGuid() == receiver_guid);
	}
	...
	*/
	...
	// TODO Create an appropiate Id for the mail (currently #1)
	MailDraft::MailDraft(String subject, String text) : m_mailTemplateId(0), m_mailTemplateItemsNeed(false), m_subject(subject),
		m_bodyId(!text.empty() ? 1 : 0), m_money(0), m_COD(0)
	{
	}

	MailDraft& MailDraft::SetSubjectAndBody(String subject, String text)
	{
		m_subject = subject;

		assert(!m_bodyId);
		m_bodyId = !text.empty() ? 1 : 0;
	...
	*/
	MailDraft& MailDraft::AddItem(GameItem* item)
	{
		m_items[guidLowerPart(item->getGuid())] = item;
		return *this;
	}
	/**
	...
	*/
	bool MailDraft::prepareItems(Player* receiver)
	{
		...
		LootInstance mailLoot(receiver, m_mailTemplateId, LOOT_MAIL);
		...
		for (LootItemList::const_iterator lootItr = lootList.begin(); lootItr != lootList.end(); ++lootItr)
		{
			if (m_items.size() < 12)
			{
				...
				if (GameItem* item = Item::CreateItem(lootitem->itemId, lootitem->count, receiver))
				{
	...
	*/
	void MailDraft::deleteIncludedItems(bool inDB /**= false*/)
	{
		...
		{
			GameItem* item = mailItemIter->second;
	...
	*/
	void MailDraft::CloneFrom(MailDraft const& draft)
	{
		...
		{
			String text = sObjectMgr.GetItemText(bodyId);
		...
		for (MailItemMap::const_iterator mailItemIter = draft.m_items.begin(); mailItemIter != draft.m_items.end(); ++mailItemIter)
		{
			...
			if (GameItem* newitem = item->CloneItem(item->GetCount()))
			{
				newitem->SaveToDB();
				AddItem(newitem);
			}
	...
	*/
	void MailDraft::SendReturnToSender(UInt32 sender_acc, UInt64 sender_guid, UInt64 receiver_guid)
	{
		...
		SendMailTo(MailReceiver(receiver, receiver_guid), MailSender(mail_message_type::Normal, sender_guid.GetCounter()), mail_check_mask::Returned, deliver_delay);
	}
	/**
	* Sends a mail.
	*
	...
	*/
	void MailDraft::SendMailTo(MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, UInt32 deliver_delay)
	{
		...
		if (sender.getMessageType() == MAIL_AUCTION && m_items.empty() && !m_money)
			expire_delay = HOUR;
		// mail from battlemaster (rewardmarks) should last only one day
		else if (sender.getMessageType() == MAIL_CREATURE && sBattleGroundMgr.GetBattleMasterBG(sender.GetSenderId()) != BATTLEGROUND_TYPE_NONE)
			expire_delay = DAY;
		...
		// Add to DB
		String safe_subject = GetSubject();
		...
		if (pReceiver)
		{
			...
			if (!m_items.empty())
			{
				for (auto mailItemIter = m_items.begin(); mailItemIter != m_items.end(); ++mailItemIter)
