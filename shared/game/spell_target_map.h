...
namespace wowpp
{
	...
	public:
		...
		const bool hasSourceTarget() const {
			return (m_targetMap & game::spell_cast_target_flags::SourceLocation) != 0;
		}
		/// Determines whether a dest location is provided.
		const bool hasDestTarget() const {
			return (m_targetMap & game::spell_cast_target_flags::DestLocation) != 0;
		}
		/// Determines whether a string target is provided.
		const bool hasStringTarget() const {
			return (m_targetMap & game::spell_cast_target_flags::String) != 0;
		}
