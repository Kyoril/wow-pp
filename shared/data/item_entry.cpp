...
namespace wowpp
{
	...
	void ItemEntry::save(BasicTemplateSaveContext &context) const
	{
		...
		if (!description.empty()) {
			context.table.addKey("desc", description);
		}
		if (itemClass != 0) {
			context.table.addKey("class", itemClass);
		}
		if (subClass != 0) {
			context.table.addKey("subclass", subClass);
		}
		if (displayId != 0) {
			context.table.addKey("display_id", displayId);
		}
		if (quality != 0) {
			context.table.addKey("quality", quality);
		}
		if (flags != 0) {
			context.table.addKey("flags", flags);
		}
		if (buyCount != 0) {
			context.table.addKey("buy_count", buyCount);
		}
		if (buyPrice != 0) {
			context.table.addKey("buy_price", buyPrice);
		}
		if (sellPrice != 0) {
			context.table.addKey("sell_price", sellPrice);
		}
		if (inventoryType != 0) {
			context.table.addKey("inv_type", inventoryType);
		}
		if (allowedClasses != -1) {
			context.table.addKey("allowed_class", allowedClasses);
		}
		if (allowedRaces != -1) {
			context.table.addKey("allowed_race", allowedRaces);
		}
		if (itemLevel != 0) {
			context.table.addKey("level", itemLevel);
		}
		if (requiredLevel != 0) {
			context.table.addKey("req_level", requiredLevel);
		}
		if (requiredSkill != 0) {
			context.table.addKey("req_skill", requiredSkill);
		}
		if (requiredSkillRank != 0) {
			context.table.addKey("req_skill_rank", requiredSkillRank);
		}
		if (requiredSpell != 0) {
			context.table.addKey("req_spell", requiredSpell);
		}
		if (requiredHonorRank != 0) {
			context.table.addKey("req_rank", requiredHonorRank);
		}
		if (requiredCityRank != 0) {
			context.table.addKey("req_city_rank", requiredCityRank);
		}
		if (requiredReputation != 0) {
			context.table.addKey("req_rep_faction", requiredReputation);
		}
		if (requiredReputationRank != 0) {
			context.table.addKey("req_rep_rank", requiredReputationRank);
		}
		if (maxCount != 0) {
			context.table.addKey("max_count", maxCount);
		}
		if (maxStack != 0) {
			context.table.addKey("max_stack", maxStack);
		}
		if (containerSlots != 0) {
			context.table.addKey("container_slots", containerSlots);
		}
		if (armor != 0) {
			context.table.addKey("armor", armor);
		}
		if (holyResistance != 0) {
			context.table.addKey("holy_res", holyResistance);
		}
		if (fireResistance != 0) {
			context.table.addKey("fire_res", fireResistance);
		}
		if (natureResistance != 0) {
			context.table.addKey("nature_res", natureResistance);
		}
		if (frostResistance != 0) {
			context.table.addKey("frost_res", frostResistance);
		}
		if (shadowResistance != 0) {
			context.table.addKey("shadow_res", shadowResistance);
		}
		if (arcaneResistance != 0) {
			context.table.addKey("arcane_res", arcaneResistance);
		}
		if (delay != 0) {
			context.table.addKey("delay", delay);
		}
		if (ammoType != 0) {
			context.table.addKey("ammo_type", ammoType);
		}
		if (bonding != 0) {
			context.table.addKey("bonding", static_cast<UInt16>(bonding));
		}
		if (lockId != 0) {
			context.table.addKey("lock_id", lockId);
		}
		if (sheath != 0) {
			context.table.addKey("sheath", static_cast<UInt16>(sheath));
		}
		if (randomProperty != 0) {
			context.table.addKey("random_property", randomProperty);
		}
		if (randomSuffix != 0) {
			context.table.addKey("random_suffix", randomSuffix);
		}
		if (block != 0) {
			context.table.addKey("block", block);
		}
		if (set != 0) {
			context.table.addKey("set", set);
		}
		if (durability != 0) {
			context.table.addKey("durability", durability);
		}
		if (area != 0) {
			context.table.addKey("area", area);
		}
		if (map != 0) {
			context.table.addKey("map", map);
		}
		if (bagFamily != 0) {
			context.table.addKey("bag_family", bagFamily);
		}
		if (material != 0) {
			context.table.addKey("material", material);
		}
		if (totemCategory != 0) {
			context.table.addKey("totem_category", static_cast<Int16>(totemCategory));
		}
		if (socketBonus != 0) {
			context.table.addKey("socket_bonus", socketBonus);
		}
		if (gemProperties != 0) {
			context.table.addKey("gem_properties", gemProperties);
		}
		if (requiredDisenchantSkill != 0) {
			context.table.addKey("req_disenchant_skill", requiredDisenchantSkill);
		}
		if (disenchantId != 0) {
			context.table.addKey("disenchant_id", disenchantId);
		}
		if (foodType != 0) {
			context.table.addKey("food_type", static_cast<UInt16>(foodType));
		}
		if (minLootGold != 0) {
		...
		if (extraFlags != 0) {
			context.table.addKey("extra_flags", extraFlags);
		}
		...
		sff::write::Array<char> statArray(context.table, "stats", sff::write::Comma);
		{
			...
			{
				if (stat.statType == 0 && stat.statValue == 0) {
					continue;
				}
		...
		sff::write::Array<char> dmgArray(context.table, "dmg", sff::write::Comma);
		{
			...
			{
				if (dmg.min == 0.0f && dmg.max == 0.0f && dmg.type == 0) {
					continue;
				}
		...
		sff::write::Array<char> spellArray(context.table, "spells", sff::write::MultiLine);
		{
			for (const auto &spell : itemSpells)
			{
				...
				sff::write::Table<char> valueTable(spellArray, sff::write::Comma);
				{
					...
					if (spell.trigger != 0) {
						valueTable.addKey("trigger", static_cast<UInt16>(spell.trigger));
					}
					if (spell.charges != 0) {
						valueTable.addKey("charges", spell.charges);
					}
					if (spell.procRate != 0.0f) {
						valueTable.addKey("proc_rate", spell.procRate);
					}
					if (spell.cooldown != 0) {
						valueTable.addKey("cooldown", spell.cooldown);
					}
					if (spell.category != 0) {
						valueTable.addKey("category", spell.category);
					}
					if (spell.categoryCooldown != 0) {
						valueTable.addKey("category_cooldown", spell.categoryCooldown);
					}
				}
				valueTable.finish();
			}
